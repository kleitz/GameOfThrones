    ...
    public void checkDead() {
        for (int i = 0; i < houses.size(); i++) {
            if (houses.get(i).isDead()) {
                if (houses.get(i).getName().equals("Stark")) {
                    Stark.reduceNum();
                }
                if (houses.get(i).getName().equals("Tully")) {
                    Tully.reduceNum();
                }
                if (houses.get(i).getName().equals("Lannister")) {
                    Lannister.reduceNum();
                }
                if (houses.get(i).getName().equals("Baratheon")) {
                    Baratheon.reduceNum();
                }
                if (houses.get(i).getName().equals("Targaryan")) {
                    Targaryan.reduceNum();
                }
                houses.remove(i);
                i--;
    ...
    private class ClickListener extends MouseAdapter {
        public void mousePressed(MouseEvent e) {
            ...
            if (houseType.equals("Stark")) {
                houses.add(new Stark("direwolf.png", p));
            } else if (houseType.equals("Baratheon")) {
                houses.add(new Baratheon("stag.png", p));
            } else if (houseType.equals("Lannister")) {
                houses.add(new Lannister("lion.png", p));
            } else if (houseType.equals("Targaryan")) {
                houses.add(new Targaryan("dragon.png", p));
            } else if (houseType.equals("Tully")) {
                houses.add(new Tully("trout.png", p));
            } else if (houseType.equals("Goku")) {
                houses.add(new Goku("goku.png", p));
            }
